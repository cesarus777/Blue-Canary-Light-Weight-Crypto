#include <assert.h>
#include <stdint.h>
#include <stdio.h>
#include <string.h>

#define COLHOZ

enum {
  Nb = 4,
  Nk = 4,
  Nr = 10,
};

enum { SBOX_SIZE = 256 };
static const uint8_t sbox[SBOX_SIZE] = {
    ,
};

static const uint8_t rc_tab[10] = {0x01, 0x02, 0x04, 0x08, 0x10,
                                   0x20, 0x40, 0x80, 0x1b, 0x36};

static void aes_rotword(uint8_t *a) {
  uint8_t t;

  t = a[0];
  a[0] = a[1];
  a[1] = a[2];
  a[2] = a[3];
  a[3] = t;
}

static void run_encryption_key_schedule(uint8_t *key, uint8_t *round_keys) {
  uint8_t i;
  uint8_t rc = 0;

  union {
    uint32_t v32;
    uint8_t v8[4];
  } tmp;

  memcpy(round_keys, key, 16);

  for (i = 4; i < 44; ++i) {
    tmp.v32 = ((uint32_t *)(round_keys))[i - 1];
    if (0 == i % 4) {
      aes_rotword((uint8_t *)&(tmp.v32));

      tmp.v8[0] = sbox[tmp.v8[0]];
      tmp.v8[1] = sbox[tmp.v8[1]];
      tmp.v8[2] = sbox[tmp.v8[2]];
      tmp.v8[3] = sbox[tmp.v8[3]];
      tmp.v8[0] ^= rc_tab[rc];
      rc++;
    }
    ((uint32_t *)(round_keys))[i] = ((uint32_t *)(round_keys))[i - 4] ^ tmp.v32;
  }
}

static inline uint8_t gmul(uint8_t a, uint8_t b) {
  uint8_t p = 0;
  uint8_t counter;
  uint8_t hi_bit_set;

  for (counter = 0; counter < 8; counter++) {
    if ((b & 1) == 1) {
      p ^= a;
    }
    hi_bit_set = (a & 0x80);
    a <<= 1;
    if (hi_bit_set == 0x80) {
      a ^= 0x1b;
    }
    b >>= 1;
  }

  return p;
}

static void encrypt_round(uint8_t *state, uint8_t *round_keys, int n) {
  assert(state);
  assert(round_keys);
  assert((n > 0) && (n <= Nr));

  uint8_t tmp[4 * Nb];

  // subBytes
  for (int i = 0; i < 4 * Nb; i++) {
    tmp[i] = sbox[state[i]];
  }
  fprintf(stderr, "  tmp: ");
  for (int i = 0; i < 16; i++)
    fprintf(stderr, "0x%02x ", tmp[i]);
  fprintf(stderr, "\n");

  // shiftRows
  for (int i = 1; i < 4; i++) {
    uint8_t swap_buf[4];
    for (int j = 0; j < 4; j++) {
      int offset = i + j * 4;
      swap_buf[j] = tmp[offset];
    }
    for (int j = 0; j < 4; j++) {
      int offset = i + j * 4;
      tmp[offset] = swap_buf[(i + j) & 3];
    }
    fprintf(stderr, "%d tmp: ", i);
    for (int k = 0; k < 16; k++)
      fprintf(stderr, "0x%02x ", tmp[k]);
    fprintf(stderr, "\n");
  }

  if (n != Nr) {
    // mixColums
    for (int i = 0; i < 4; ++i) {
      uint8_t t =
          tmp[4 * i + 0] ^ tmp[4 * i + 1] ^ tmp[4 * i + 2] ^ tmp[4 * i + 3];

      state[4 * i + 0] =
          gmul(2, tmp[4 * i + 0] ^ tmp[4 * i + 1]) ^ tmp[4 * i + 0] ^ t;

      state[4 * i + 1] =
          gmul(2, tmp[4 * i + 1] ^ tmp[4 * i + 2]) ^ tmp[4 * i + 1] ^ t;

      state[4 * i + 2] =
          gmul(2, tmp[4 * i + 2] ^ tmp[4 * i + 3]) ^ tmp[4 * i + 2] ^ t;

      state[4 * i + 3] =
          gmul(2, tmp[4 * i + 3] ^ tmp[4 * i + 0]) ^ tmp[4 * i + 3] ^ t;
      fprintf(stderr, "%d state: ", i);
      for (int k = 0; k < 16; k++)
        fprintf(stderr, "0x%02x ", state[k]);
      fprintf(stderr, "\n");
    }
  } else {
    fprintf(stderr, "state: ");
    for (int i = 0; i < 16; i++)
      fprintf(stderr, "0x%02x ", state[i]);
    fprintf(stderr, "\n");
  }

  // addKey
  for (int i = 0; i < 4 * Nb; i++) {
    state[i] = (n == Nr ? tmp[i] : state[i]) ^ round_keys[4 * Nb * n + i];
  }
  fprintf(stderr, "state: ");
  for (int i = 0; i < 16; i++)
    fprintf(stderr, "0x%02x ", state[i]);
  fprintf(stderr, "\n");
}

static void encrypt_impl(uint8_t *block, uint8_t *round_keys) {
  assert(block);
  assert(round_keys);

  for (int i = 0; i < 4 * Nb; i++) {
    block[i] ^= round_keys[i];
  }

  for (int i = 1; i <= Nr; i++) {
    fprintf(stderr, "round %2d\n", i);
    encrypt_round(block, round_keys, i);
  }
}

void encrypt(uint8_t *block) {
  assert(block);
  uint8_t expected_key[16] = {0xf5, 0x30, 0x35, 0x79, 0x68, 0x57, 0x84, 0x80,
                              0xb3, 0x98, 0xa3, 0xc2, 0x51, 0xcd, 0x10, 0x93};
  uint8_t round_keys[16 * 11];
  run_encryption_key_schedule(expected_key, round_keys);
  encrypt_impl(block, round_keys);
}
